...
$(document).ready(() => {
    ...
    function loadTable(fittedType, bodyType, pattern) {
        ...
        const headers1 = pattern.patterns.map(k => k.conditions.chest);
        const headers2 = pattern.patterns.map(k => k.conditions.seat);
        const vertHeaders = pattern.patterns.map(k => k.conditions.height);
